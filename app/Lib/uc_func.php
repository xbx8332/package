		...
		if ($user_id > 0){
			...
			if($count >0){
				...
				foreach($list as $k=>$v){
					...
					$list[$k]['u_load_money_format'] =  format_price($list[$k]['u_load_money']/10000)."万";
...
//					}
	...
	function getUcTransferList($page,$status){
		...
		$condition = ' AND (d.repay_start_time + d.transfer_day + 24*3600) < '.TIME_UTC.' and d.is_effect=1 and d.is_delete=0  and d.repay_time_type =1 and  d.publish_wait=0 and dl.user_id='.$GLOBALS['user_info']['id']."  ";
	...
	function getUcToTransfer($id,$tid){	
		...
		$condition = ' AND (d.repay_start_time + d.transfer_day + 24*3600) < '.TIME_UTC.' AND dl.id='.$id.' AND d.deal_status = 4 and d.is_effect=1 and d.is_delete=0  and d.repay_time_type =1 and  d.publish_wait=0 and dl.user_id='.$GLOBALS['user_info']['id']."  and d.next_repay_time - ".TIME_UTC." + 24*3600 - 1 > 0  ";
		if($tid > 0)
		{
	...
	function getUcDoTransfer($id,$tid,$paypassword,$transfer_money){
		...
		if($transfer){
			...
			$transfer_data['near_repay_time'] = $transfer['last_repay_time'] > 0 ? $transfer['last_repay_time'] : $transfer['repay_start_time'];
	...
	function getUcSaveCarry($amount,$paypassword,$bid){
		...
		if($GLOBALS['user_info']['id'] > 0){
			...
			//更新会员账户信息
			if((floatval($fee) + floatval($data['money'])) <= get_user_info("AES_DECRYPT(money_encrypt,'".AES_DECRYPT_KEY."')-nmc_amount","id=".intval($GLOBALS['user_info']['id']),"ONE")){
