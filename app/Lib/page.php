...
class Page  {
   ...
   private function get_page_link($url,$page)
    ...
    public function show() {
        ...
        if ($upRow>0){
            $upPage="<a href='".$this->get_page_link($url,$upRow)."' class='prev'>".$this->config['prev']."</a>";
        }else{
            $upPage="";
        }

        if ($downRow <= $this->totalPages){
            $downPage="<a href='".$this->get_page_link($url,$downRow)."' class='next'>".$this->config['next']."</a>";
        }else{
            $downPage="";
        }
        // << < > >>
        if($nowCoolPage == 1){
            ...
            $prePage = "<a href='".$this->get_page_link($url,$preRow)."'  class='paginate_button previous'>上".$this->rollPage."页</a>";
            $theFirst = "<a href='".$this->get_page_link($url,1)."' class='first'>".$this->config['first']."</a>";
        }
        if($nowCoolPage == $this->coolPages){
            ...
            $nextPage = "<a href='".$this->get_page_link($url,$nextRow)."' class='paginate_button next'>下".$this->rollPage."页</a>";
            $theEnd = "<a href='".$this->get_page_link($url,$theEndRow)."' class='last'>".$this->config['last']."</a>";
        }
        // 1 2 3 4 5
        $linkPage = "";
