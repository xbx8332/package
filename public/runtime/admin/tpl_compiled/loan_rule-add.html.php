...
<article class="page-container">
	...
	<div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="" placeholder="" id="adminName" name="rule_name">
		</div>
	</div>
	<div class="row cl">
		<label class="form-label col-xs-4 col-sm-3">贷款规则：</label>
		<div class="formControls col-xs-8 col-sm-9">
			<textarea name="rulemsg" cols="" rows="" class="textarea" dragonfly="true" onKeyUp="$.Huitextarealength(this,100)"><?php echo $this->_var['data']['rule_msg']; ?></textarea>
			
		</div>
	</div>
	<div class="row cl">
		<label class="form-label col-xs-4 col-sm-3">规则类型：</label>
		<div class="formControls col-xs-8 col-sm-9"> <span class="select-box" style="width:150px;">
			<select class="select" name="type" size="1">
				
				<option value="1">贷款规则</option>
				<option value="2">贷款资格</option>
				<option value="3">快速问答</option>
			</select>
			</span> </div>
	</div>
	...
	$("#form-admin-add").validate({
		...
		submitHandler:function(form){
			$(form).ajaxSubmit({
				type: 'post',
				url: "<?php
echo parse_url_tag("u:admin|loanmanage#rulesave|"."".""); 
?>" ,
