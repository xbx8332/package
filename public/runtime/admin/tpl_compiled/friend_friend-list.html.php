...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 会员管理 <span class="c-gray en">&gt;</span> 好友邀请列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	
	<!-- <div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a href="javascript:;" onclick="member_add('添加用户','member-add.html','','510')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加用户</a></span>  </div> -->
	<div class="mt-20">
...
?>
			<tr class="text-c">
				<td>
					<div class="btn-group">
	  					<span class="btn btn-default radius" onclick="friend_invitor('<?php echo $this->_var['user']['id']; ?>',this)" >
	  						<i class="Hui-iconfont">&#xe600;</i>
	  					</span>
					</div>
					<div class="btn-group" style="display:none">
	  					<span class="btn btn-default radius" onclick="friend_hide('<?php echo $this->_var['user']['id']; ?>',this)" >
	  						<i class="Hui-iconfont">&#xe6a1;</i>
	  					</span>
					</div>
				</td>
				...
				<td class="text-l"><?php echo $this->_var['user']['ref']; ?></td>
...
?></td>
				...
				<a style="text-decoration:none" class="ml-5" onClick="member_stop(this,<?php echo $this->_var['user']['id']; ?>)" href="javascript:;" title="启用"><i class="Hui-iconfont">&#xe6a7;</i></a>
...
/*查询用户受邀人列表  */
function friend_invitor(uid,dobj)
{
	var obj = {};
	obj.id=uid;
	$.ajax({
		type:'POST',
		url:"<?php
echo parse_url_tag("u:admin|friendmanage#friend_invitor|"."".""); 
?>",
		dataType:'json',
		data:obj,
		success:function(res)
		{
			...
			{
				var list = res.data;
				relate_fri(list,uid,dobj);
					
			}else{
				layer.alert("该用户暂无邀请人列表",{icon:5})
			}
...
/*处理好友添加收起结点  */
function relate_fri(list,uid,dobj)
{
	$(dobj).parent().hide().next().show();
	for(var i=0;i<list.length;i++)
	{
		var is_effect = list[i].is_effect;
		if(parseInt(is_effect))
		{
			var d_s = "success", txt = "已启用";
		}else{
			var d_s = "default", txt = "已禁用";
		}
		var is_eHtml = $("<span class='label label-"+d_s+" radius'>"+txt+"</span>");
		var html = $("<tr class='text-c f_"+uid+" success'><td></td><td>"+list[i].id+"</td><td>"+list[i].user_name+"</td><td>"+list[i].mobile+"</td><td>"+list[i].email+"</td><td>"+list[i].ref+"</td><td></td><td>"+list[i].create_date+"</td><td><span class='label label-"+d_s+" radius'>"+txt+"</span></td><td>无</td></tr>");
		html.insertAfter($(dobj).parent().parent().parent());
	}
}


/*收起用户好友邀请列表  */
function friend_hide(uid,dobj)
{
	$(dobj).parent().hide().prev().show();
	var flag = $(dobj).parent().parent().parent().nextAll(".f_"+uid).slideUp(100).remove();
}
