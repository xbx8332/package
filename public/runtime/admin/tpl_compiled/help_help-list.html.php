...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 帮助中心 <span class="c-gray en">&gt;</span> 文章列表 <a class="btn btn-success radius r btn-refresh" style="line-height:1.6em;margin-top:3px" href="javascript:void(0);" onclick="refresh();" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container">
	
	<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="deldata()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a href="javascript:;" onclick="admin_add('添加文章','<?php
echo parse_url_tag("u:admin|help#wenzhangadd|"."".""); 
?>','800','500')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加文章</a></span> </div>
...
?>
			<tr class="text-c">
				...
				<td>
				<a title="编辑" href="javascript:;" onclick="wenzhang_edit('管理员编辑','<?php
echo parse_url_tag("u:admin|help#wenzhangedit|"."".""); 
?>','<?php echo $this->_var['list']['id']; ?>','800','500')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
				<a title="删除" href="javascript:;" onclick="wenzhang_del(this,<?php echo $this->_var['list']['id']; ?>)" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
			</tr>
	...
	layer.confirm('确认要批量删除吗？',function(index){
		 $('input:checkbox:checked').each(function() {
			...
			$.ajax({
				type: 'POST',
				url: "<?php
echo parse_url_tag("u:admin|help#help_del|"."".""); 
?>",
				dataType: 'json',
				data:{
					'ids':checked
...
function wenzhang_del(obj,id){
	...
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'POST',
			url: '<?php
echo parse_url_tag("u:admin|help#wenzhangdel|"."".""); 
?>',
			dataType: 'json',
			data:pro,
			success: function(data){
...
/*管理员-编辑*/
function wenzhang_edit(title,url,id,w,h){
	var url = url+"&id="+id;
	layer_show(title,url,w,h);
}
/*管理员-停用*/
function admin_stop(obj,id){
	layer.confirm('确认要停用吗？',function(index){
		...
		$.ajax({
			type: 'POST',
			url: '<?php
echo parse_url_tag("u:admin|help#wenzhangstop|"."".""); 
?>',
			dataType: 'json',
			success: function(data){
				$(obj).parents("tr").remove();
...
function admin_start(obj,id){
	layer.confirm('确认要启用吗？',function(index){
		...
		$.ajax({
			type: 'POST',
			url: '<?php
echo parse_url_tag("u:admin|help#wenzhangstart|"."".""); 
?>',
			data:pro,
			dataType: 'json',
			success: function(data){
