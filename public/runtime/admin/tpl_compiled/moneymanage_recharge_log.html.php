...
<body >
	<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>资金管理 <span class="c-gray en">&gt;</span> 充值日志 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
...
?>
				<tbody>
					<tr class="text-c va-m">
						...
						<td><?php echo $this->_var['list']['operation_admin']; ?></td>
						
						<td>￥<?php echo $this->_var['list']['recharge_price']; ?></td>
						<td><?php echo $this->_var['list']['user_name']; ?></td>
						<!-- 
						<td class="td-status"><?php if ($this->_var['list']['status'] == 1): ?><span class="label label-success radius">通过</span><?php elseif ($this->_var['list']['status'] == 2): ?><span class="label label-default radius">拒绝</span><?php endif; ?></td> -->
						<!-- 
						<td class="td-manage"><a style="text-decoration:none" class="ml-5" onClick="withdraw_del(this,'<?php echo $this->_var['list']['id']; ?>')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
						 -->
...
/*产品-删除*/
function over_suess(obj,id){
		pro={};
		pro.id=id;
		$.ajax({
			type: 'POST',
			url: '<?php
echo parse_url_tag("u:admin|f_licai#over_suess|"."".""); 
?>',
			dataType: 'json',
			data:pro,
			success: function(data){
				alert(data.msg);
				if(data.status==1){
					$(obj).parents("tr").find(".td-status").html('<span class="label label-defaunt radius">已赎回</span>');
				}
			},
			error:function(data) {
