...
?>
					<tr class="text-c va-m">
						...
						<td class="td-manage"> <a style="text-decoration:none" class="ml-5" onClick="rate_del(this,'<?php echo $this->_var['list']['id']; ?>')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
...
/*浮动查看  */
function float_check(title,url,id,w,h){
	var index = layer.open({
		type: 2,
		title: title,
		content: url
...
function rate_del(obj,id){
	layer.confirm('确认要删除该利率吗？',
		...
		$.ajax({
			...
			data:{id:id},
			url: '<?php
echo parse_url_tag("u:admin|licai#rate_del|"."".""); 
?>',
			dataType: 'json',
