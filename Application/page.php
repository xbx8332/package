   ...
   private function get_page_link($url,$page)
    ...
    public function show() {
       ...
       // if(app_conf("URL_MODEL")==1)$url = $GLOBALS['current_url'];
        ...
        $this->nexturl = $this->get_page_link($url,$downRow);
       
        if ($upRow>0){
            $upPage="<a href='".$this->get_page_link($url,$upRow)."' class='btn2'>".$this->config['prev']."</a>";
        }else{
            $upPage="";
        }

        if ($downRow <= $this->totalPages){
            $downPage="<a href='".$this->get_page_link($url,$downRow)."' class='btn2'>".$this->config['next']."</a>";
        }else{
            $downPage="";
        }
        // << < > >>
        ...
        return trim("<div class='csq'>".$pageStr."<div/>");
