...
class AppTemplate
{
    ...
    {
    	if(IS_DEBUG)
        $this->_errorlevel = E_ALL^E_NOTICE^E_WARNING;//$this->_errorlevel = E_ALL^E_NOTICE^E_WARNING^E_DEPRECATED^E_STRICT;
        else 
        $this->_errorlevel = 0;
        $this->_nowtime    = time();
    ...
    function es_tmpl($out)
    {
	        ...
	        $city_condition = " and ( city_ids REGEXP '^".$city_id."$|^".$city_id.",|,".$city_id."$|,".$city_id.",' or city_ids = '') order by city_ids desc";
	        ...
	        {    
	        	if(trim($layout_array[3][$k])!='/') //有关联数据
	        	{
	        		...
	        		$adv = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."adv where tmpl='".app_conf("TEMPLATE")."' and adv_id='".$adv_id."' and rel_table = '".$rel_array[1][0]."' and rel_id = ".intval($rel_array[3][0])." and is_effect = 1".$city_condition,true);
	        	}	
	        	else
	        	{
	        		$adv = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."adv where tmpl='".app_conf("TEMPLATE")."' and adv_id='".$adv_id."' and is_effect = 1".$city_condition,true);      	
	        	}
	        	if($adv['city_ids']!='')
	        	{
	        		$city_ids = explode(',',$adv['city_ids']);
	        		if(in_array($GLOBALS['city']['id'],$city_ids))
	        		{
