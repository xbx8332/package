...
$dist_cfg = APP_ROOT_PATH."sys/dist_cfg.php";
...
//引入数据库的系统配置及定义配置函数
$cfg_file = APP_ROOT_PATH.'sys/config.php';
if(file_exists($cfg_file)){
    $sys_config = require_once APP_ROOT_PATH.'sys/config.php';
}
...
$define_file = APP_ROOT_PATH."sys/define.php";
...
if(!function_exists("load_fanwe_cache")){
    function load_fanwe_cache(){
        ...
        if(file_exists(APP_ROOT_PATH."sys/cache/".$cacheClass.".php")){
            require_once APP_ROOT_PATH."sys/cache/".$cacheClass.".php";
            if(class_exists($cacheClass)){
                $cache = new $cacheClass();
            }
            return $cache;
        }else{
            $file_cache_file = APP_ROOT_PATH.'sys/cache/CacheFileService.php';
            if(file_exists($file_cache_file)){
                require_once APP_ROOT_PATH.'sys/cache/CacheFileService.php';
            }
...
$cache_service_file = APP_ROOT_PATH."sys/cache/Cache.php";
...
//定义DB
$db_cls_file =  APP_ROOT_PATH."sys/db/db.php";
...
//定义模板引擎
$tmpl_cls_file = APP_ROOT_PATH.'sys/template/template.php';
if(file_exists($tmpl_cls_file))
{
    require_once  $tmpl_cls_file;
    if(class_exists("AppTemplate"))
    {
        if(!file_exists(APP_ROOT_PATH.'public/runtime/home/tpl_caches/'))
            mkdir(APP_ROOT_PATH.'public/runtime/home/tpl_caches/',0777);
        if(!file_exists(APP_ROOT_PATH.'public/runtime/app/tpl_compiled/'))
            mkdir(APP_ROOT_PATH.'public/runtime/home/tpl_compiled/',0777);
        if(!file_exists(APP_ROOT_PATH.'public/runtime/admin/tpl_caches/'))
        	mkdir(APP_ROOT_PATH.'public/runtime/admin/tpl_caches/',0777);
        if(!file_exists(APP_ROOT_PATH.'public/runtime/admin/tpl_compiled/'))
        	mkdir(APP_ROOT_PATH.'public/runtime/admin/tpl_compiled/',0777);
    ...
    function syn_to_remote_image_server($url)
    {
        if($GLOBALS['distribution_cfg']['OSS_TYPE']&&$GLOBALS['distribution_cfg']['OSS_TYPE']!="NONE")
        {
            ...
            elseif($GLOBALS['distribution_cfg']['OSS_TYPE']=="ALI_OSS")
            {
                ...
                $ali_oss_sdk = APP_ROOT_PATH."sys/alioss/sdk.class.php";
...
if(!class_exists("FanweSessionHandler"))
{
    class FanweSessionHandler
    {
        ...
        function open($savePath, $sessionName)
        {
            ...
            {
                $this->mem = require_once APP_ROOT_PATH."sys/cache/MemcacheSASL/MemcacheSASL.php";
...
{
    $nav_list =require APP_ROOT_PATH."sys/biz_cfg/".APP_TYPE."/m_biznav_cfg.php";
...
{
    $nav_list =require APP_ROOT_PATH."sys/biz_cfg/".APP_TYPE."/biznav_cfg.php";
    if($GLOBALS['account_info']['is_main']||$GLOBALS['account_info']['is_staff']==1)
    {
        foreach($nav_list as $k=>$v)
        {
...
//     require_once APP_ROOT_PATH.'/sys/libs/crypt_aes.php';
//     $aes =new CryptAES();
...
function send_schedule_plan($type,$name,$schedule_data,$schedule_time,$dest="")
{
    ...
    $GLOBALS['db']->autoExecute(DB_PREFIX."point_schedule_list",$data);
...
function exec_schedule_plan($schedule_data)
{
    ...
    require_once APP_ROOT_PATH . 'sys/schedule/' . $cname . ".php";
    $c = new $cname;
