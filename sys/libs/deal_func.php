...
function format_deal_item(&$deal,$user_name="",$user_pwd=""){
	...
	//还需多少钱
	$deal['need_money'] = format_price($deal['borrow_amount'] - getCollWaitMoney($deal) - $deal['load_money']);//计算锁定了的钱
	//百分比
	...
	if($deal['uloadtype'] == 1){
		...
		$deal['need_portion'] = intval(($deal['borrow_amount'] - $deal['load_money'] - getCollWaitMoney($deal)) / $deal['min_loan_money']);
	}	
...
function get_deal_load_list($deal){
	...
	foreach($deal_repay_list as $k=>$v){
        ...
        //抵押物管理费
        $loan_list[$i]['mortgage_fee'] = $v['mortgage_fee'] - $v['true_manage_money'];
...
function check_dobid2($deal_id,$bid_money,$bid_paypassword,$is_pc = 0,$ecv_id=0){	
...
function dobid2($deal_id,$bid_money,$bid_paypassword,$is_pc=0,$ecv_id=0){
	$root = check_dobid2($deal_id,$bid_money,$bid_paypassword,$is_pc,$ecv_id);
	if ($root["status"] == 0){
		return $root;
	}
	elseif($root["status"] == 2){
		$root['jump'] = APP_ROOT."/index.php?ctl=collocation&act=RegisterCreditor&deal_id=$deal_id&user_id=".$GLOBALS['user_info']['id']."&bid_money=".$root['bid_money']."&bid_paypassword=$bid_paypassword"."&from=".$GLOBALS['request']['from']."&ecv_id=".$ecv_id;		
	...
	if($load_id > 0){
		...
		$ecv_money = 0;
		if($ecv_id > 0){
			$GLOBALS['db']->query("UPDATE ".DB_PREFIX."ecv SET use_count = use_count + 1 WHERE (begin_time =0 OR (begin_time > 0 AND begin_time < ".TIME_UTC.")) AND (end_time = 0 OR (end_time > 0  AND (end_time +24*3600 - 1) > ".TIME_UTC.")) AND (use_limit =0  OR (use_limit >0 AND use_limit - use_count > 0)) AND id=".$ecv_id." AND user_id=".$GLOBALS['user_info']['id']);
			if($GLOBALS['db']->affected_rows()){
				$ecv_money = $GLOBALS['db']->getOne("SELECT `money` FROM ".DB_PREFIX."ecv WHERE id=".$ecv_id);
			}
		}
		//更改资金记录
		if($bid_money - $ecv_money > 0){
			$msg = '[<a href="'.$root['deal']['url'].'" target="_blank">'.$root['deal']['name'].'</a>]的投标,付款单号'.$load_id;
			require_once APP_ROOT_PATH."system/libs/user.php";
			modify_account(array('money'=>-($bid_money - $ecv_money),'lock_money'=>($bid_money - $ecv_money)),$GLOBALS['user_info']['id'],$msg,2);
		}
